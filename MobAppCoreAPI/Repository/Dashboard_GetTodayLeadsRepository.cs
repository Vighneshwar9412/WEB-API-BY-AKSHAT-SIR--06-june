...
{
    public class Dashboard_GetTodayLeadsRepository : IDashboard_GetTodayLeads
    {
        public async Task<dynamic> getTodayLeads(HttpRequest req,int page_index,int page_size)
        {
            ...
            try
            {
                ...
                generalResponse.Data = await (new DashboardBLL()).GetDashboardTodayLeads(mKey, loginId,page_index,page_size);

                return generalResponse;
            }
            catch (Exception ex)
            {
                Utility.LogErrorText(ex.ToString(), "v1/dashboard-leads");
